...
Booking.com - итак все знают, внутри:
...
Пришел бизнес и сказал faster time to market, много идей и фич выводить на рынок.

Раньше чтобы сделать что-то новое, нужна новая Server role - паппет, порты, сд, квм, базы, лб, деплой - ручной, занимало дни и недели, взаим по jira.

Доклад про быстрое выведение продуктов.
Путь в облака был тяжелый и чтобы не повторять ошибок.

Было 3 итерации - марафон, опеншифт и k8s.

Как профакапили 2.5 платформы, 1 и 2 точно профокапили, третья с лета и вроде все работает, но уверенности нет. И через год возможно booking.com расскажет как профакапили и 3 платформу.

Marathon.
Хакатон в середине 2016 года, но все еще не закончился. Набор скриптов. 2 шаренных кластера. Десятки проектов. RBAC не было, сложности с fw, поэтому отказались. 


Мысль 1. Не стоит недооценивать способность кода выживать. Сейчас поддерживают все 3 платформы. RIP 12.2018.


OpenShift.
Включает k8s, Ansible, Open vSwitch (network level isolation, pci, динамические дырки в fw, сильно полагались, думали решит все проблемы).
...
* функциональные тесты для k8s и интеграций, cron джобы которые каждую минуту проверяют функционал k8s
