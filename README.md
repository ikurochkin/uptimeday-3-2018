...
Проблемы с ростом:
1 итерация - На каждом сервере запущено все (cassandra, elastic, сервисы), управление в ansible

2 итерация - вынесли elastic на отдельные машины, нужны ресурсы

3 итерация - in memory cash метрик, нужны ресурсы и так далее
...
Потом появилась kafka, потом nginx как единая точка входа, появился clickhouse.
