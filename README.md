...
* БД и персистентные хранилища на выд серверах
...
Управление ресурсами. Куда пристраивать новый сервис? Смотрели глазами, выбирали вручную железки, смотрели inventory и в мониторинге. И вроде как в k8s это уже есть, request+limit то что нужно. Ввязываемся, чтобы *управлять ресурсами*.
...
Правильно спроектированная СОА подразумевает автономность сервисов, но на практике это похоже на спрут - запутанная модель данных, транспорт, сеть. Это неухоженная СОА. 


Второй антипример - монолит. Сильная внутренняя связность, транспорт зашитый в приложение, функциональные единицы чувствительны к транспорту, без ретраев и лейтенси тайм.
...
Все уменьшалось, минимальная инфраструктура, потом началась фаза роста. Новые микросервисы, фичи в новых микросервисах. Ресурсов стало не хватать. Хостились в DO, увеличение ресурсов там медленоое. Быстрее было взять новый сервер, но работа была полуручная. 
...
По  ClickHouse. Ограничили пул соединений. Взяли go migrate для миграций, так как flyway не поддерживается.
...
Доклад про то, как в ITSumma меняли идеалогию, с какими проблемами столкнулись и полюбили k8s.
...
* Поддержка инфраструктуры (24x7)
* Организация работы команды (много разработчиков и админов)
...
* fpm-base контейнер с библиотеками и зависимостями, это базовый образ. Из него собирают fpm образ - from node, from base
...
* k8s cronjobs (fpm-based). Первый инцидент - кроны не успевали пройти и после определенного времени k8s падал. Переработали cron таски и стало хорошо.
* 2 PV модуля:
  * PV для статики
...
* Введение новых фич интересно быстро и увлекательно
...
* Непонятное поведение, как коробочный проект не взлетело, нужно разбираться что внутри - примеры, пропадали правила в open vswitch, iptables под тысячу строк, дебаг вручную. Рестарт всех контейнеров на ноде. Одна нода решила взять нагрузку в кластере - 356 подов, 1.5к контейнеров.
...
Было 2 фундаментальные проблемы

Проблема 1. Хотели простой и удобный интерфейс, аля heroku.
...
* Мы хотим нарастить внутреннюю экспертизу
* Мы не хотим вендор lock
...
* Сеть и правильное разделение доступа сервисов, deis для упрощения жизни пользователей, heroku style
...
"Нельзя просто так взять и перевести монолит, если у вас есть план, то его нужно умножить на 10". 
...
* Которые нельзя прогнозировать - темная материя - и она больше SLA в 52 минуты.
